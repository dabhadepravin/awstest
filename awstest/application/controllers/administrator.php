...
class Administrator extends CI_Controller {
	...
	{
		$selected['sel_tab'] = "dashboard";
		$this->load->view('header',$selected);
	...
	public function dashboard(){
		...
		$data['orders'] = $orders;
		//get list of users - advertisers 
		$users = $this->user_model->getAdvertisers();
		$data['users'] = $users;
		$selected['sel_tab'] = "dashboard";
		$this->load->view('header',$selected);
	...
	public function settings(){
		$data['cuser'] = $this->session->userdata('advtid');
		//get users orders
		$settings = $this->administrator_model->get_settings();
		$selected['sel_tab'] = "settings";
	...
	public function usettings(){
		...
		if($_POST['submit']=="Save"){
			$settings = array_splice($_POST,1);
			$this->administrator_model->save_settings($settings);
			$settings = $this->administrator_model->get_settings();
			$data['settings'] = $settings;
	...
	public function apporveuser(){
		$userid = $_GET['uid'];
		print_r($_GET);
		//get user information 
		$compnayName = '';
		$userinfo = $this->user_model->getAdvertisers($userid);
		if(count($userinfo)>0){
			$user = $userinfo[0];
			$compnayName = $user->compnayname;
		}
		
		$this->gahelper->createServiceObject("CompanyService");
		$retdata = $this->gahelper->checkAdvertiserExists($compnayName);
		if($retdata == true){
			$advtdata = array();
			$advtdata['companyname'] = $user->compnayname;
			$advtdata['phone'] = $user->phone;
			$advtdata['fax'] = $user->phone;
			$advtdata['email'] = $user->email;
			$advtdata['address'] = "";
			$advtdata['adveriser_internal_id'] = $user->id;
			$dfpid = $this->gahelper->createAdvertiser($advtdata);
			if(!empty($dfpid)){
				$this->user_model->updateDFPuserid($user->id,$dfpid);
			}

		}
		//checkAdvertiserExists
